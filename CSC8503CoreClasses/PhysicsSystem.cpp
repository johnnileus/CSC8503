...
void PhysicsSystem::ImpulseResolveCollision(
	...
	float cRestitution = (elasticityB + elasticityA) / 2; // disperse some kinetic energy
	...
	tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data) {
			...
			for (auto i = data.begin(); i != data.end(); ++i) {
				for (auto j = std::next(i); j != data.end(); ++j) {
					// Check if this pair of items is already in the collision set
					// Avoid duplicating pairs in multiple quadtree nodes
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		if (applyGravity && inverseMass > 0 && object->GetGravity()) {
		...
		object->UpdateInertiaTensor(); // Update tensor based on orientation
